...
> **实现状态**：M1 切片已编码 + 本机 lab（**未**打 `vistacast-v0.1.0`）；M2 必须切片 + P1 已编码（**未**打 `v0.2.0`）。检测默认 stub，预览为 JPEG。**不再**等待 DataLuminary / BlockyEdu P0。  
> **详细 spec**：[VistaCast/spec](https://github.com/VistaCast/VistaCast/tree/main/spec) · 手册：[m1](https://github.com/VistaCast/VistaCast/blob/main/spec/m1-commercial-playbook.md) · [m2](https://github.com/VistaCast/VistaCast/blob/main/spec/m2-sentinel-playbook.md) · 诚实矩阵：[implementation-status.md](https://github.com/VistaCast/VistaCast/blob/main/spec/implementation-status.md)
...
| **M3** | Embedded | OEM / 看护 | SDK、级联通知 | ⬜ 商务门闩 |
