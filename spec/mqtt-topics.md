...
camera↔device 是可选 payload 字段（VistaCast FR-ECO-03），**不**改 topic。SyncroBrain 可订阅本总线做仓储/楼宇联动；家庭 `care` **禁止**上本 topic。生产入口仍是签名 Webhook。
...
SyncroBrain 可选订阅（`EVENT_BUS_MQTT_URL`，**不是** ThingsBoard `:1883`）。`care` 载荷必须由发布端跳过、由消费端拒绝。摄像头若绑定了 `syncrobrainDeviceId`，仅写入 `alert.v1.payload`，topic 不变。**不是** TB `v1/devices/me/telemetry`。权威生产路径是 SyncroBrain HMAC Webhook。
...
- 家庭看护或自动急救语义（`alert.v1.care` 不得出现在本 topic）
...
| 2026-09-05 | 1.1.0 | SyncroBrain 可选消费者；care 禁止 MQTT；Webhook 为生产入口 |
